$('#voteBugModal').on('show.bs.modal', function (event) {
    ...
    if (privilegeLevel === 'A') {
        ...
        listHTML += `<button type="button" class="btn btn-primary dialogButton" onclick="deleteVotesBug(` + myIndex + `)">Clear</button>`
    }
    listHTML += `</div>`
    listHTML += `</div>`
...
$('#editBugModal').on('show.bs.modal', function (event) {
    ...
    document.getElementById('edit-bug-vote').innerHTML = listHTML;
